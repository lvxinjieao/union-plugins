...
public class UDAgent {
	...
	public void init(final Activity context){
		...
		try{
			final UDevice device = UDManager.getInstance().collectDeviceInfo(context, U8SDK.getInstance().getAppID(), U8SDK.getInstance().getCurrChannel(), U8SDK.getInstance().getSubChannel());
			if(device == null){
				Log.e("union", "collect device info failed");
				return;
			}
			...
			Log.e("union", "submit device info failed.\n"+e.getMessage());
			e.printStackTrace();
		}
	 ...
	 * @param user
	 */
	public void submitUserInfo(final Activity activity, final UserExtraData user){
		...
		try{
			...
			if(sendable){
				...
				log.setDeviceID(GUtils.getDeviceID(activity));
				...
				SubmitTask task = new SubmitTask(new ISubmitListener() {
					...
					@Override
					public void run() {
						UDManager.getInstance().submitUserInfo(activity, U8SDK.getInstance().getAnalyticsURL(), U8SDK.getInstance().getAppKey(), log);
					}
				});
				task.execute();	
			}
